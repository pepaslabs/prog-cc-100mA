#ifndef _FEATURES_H_
#define _FEATURES_H_

//#define HAS_HARDWARE_SPI
//#define HAS_USI_SPI

#define HAS_BOOT_MESSAGE

#define HAS_INCREMENT_COMMAND
#define HAS_DECREMENT_COMMAND

#define HAS_CURRENT_COMMAND
#define HAS_CURRENT_COMMAND_DEBUGGING

#define HAS_CODE_COMMAND

#define HAS_ERROR_PRINTING
#define HAS_SUCCESS_PRINTING

#define HAS_CALIBRATE_ZERO_COMMAND
#define HAS_CALIBRATE_ZERO_COMMAND_DEBUGGING

#define HAS_CALIBRATE_FULL_SCALE_COMMAND
#define HAS_CALIBRATE_FULL_SCALE_COMMAND_DEBUGGING

#define HAS_EEPROM_BACKED_CALIBRATION_VALUES

#define HAS_DUMP_COMMAND

#define HAS_RUNTIME_VERBOSITY_CONFIG

#endif

